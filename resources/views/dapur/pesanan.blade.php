...
<title>Dapur — Pesanan Masuk</title>
...
.poll-badge { font-size: 12px; color: var(--green); background: var(--green-bg); border: 1px solid #a7f3d0; padding: 5px 12px; border-radius: 20px; font-family: 'Inter', sans-serif; font-weight: 600; }
...
.order-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 3px; background: linear-gradient(90deg, #10b981, #34d399); }
.order-card:hover { box-shadow: var(--shadow-md); transform: translateY(-3px); }
.order-card-top { padding: 18px 20px; border-bottom: 1px solid var(--border); background: var(--green-bg); display: flex; align-items: center; justify-content: space-between; gap: 12px; }
.oc-left { display: flex; align-items: center; gap: 14px; }
.table-badge { width: 48px; height: 48px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 22px; flex-shrink: 0; background: #d1fae5; border: 1px solid #a7f3d0; }
...
.timer-wrap { padding: 10px 20px; background: var(--green-bg); border-bottom: 1px solid #a7f3d0; display: flex; align-items: center; gap: 8px; }
.timer-label { font-size: 11.5px; font-weight: 700; color: var(--green-text); font-family: 'Inter', sans-serif; text-transform: uppercase; letter-spacing: 0.5px; }
.timer { font-size: 14px; font-weight: 800; color: var(--green); font-family: 'Inter', sans-serif; font-variant-numeric: tabular-nums; margin-left: auto; }
.timer.warning { color: var(--red); }
.order-items { padding: 16px 20px; }
.item-row { display: flex; align-items: flex-start; gap: 10px; font-size: 13px; padding: 9px 0; border-bottom: 1px dashed var(--border); font-family: 'Inter', sans-serif; }
.item-row:last-child { border-bottom: none; }
.item-qty { width: 26px; height: 26px; border-radius: 7px; background: #d1fae5; border: 1px solid #a7f3d0; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 800; color: var(--green); flex-shrink: 0; }
.item-name { font-weight: 600; color: var(--text-primary); }
.item-notes { font-size: 11.5px; color: var(--amber); margin-top: 2px; }
.order-note { padding: 10px 20px; background: var(--amber-bg); border-top: 1px solid #fde68a; font-size: 12.5px; color: var(--amber-text); }
...
.ab-green { background: linear-gradient(135deg, #10b981, #059669); color: white; box-shadow: 0 3px 10px rgb(5 150 105/.28); }
.ab-green:hover { box-shadow: 0 6px 16px rgb(5 150 105/.38); filter: brightness(1.07); transform: translateY(-1px); }
  ...
  <div class="header-right">
    <div class="profile-wrap">
      <div class="user-btn" id="profileBtn" onclick="toggleDropdown()">
        ...
        <div class="user-info">
          <div class="user-name">Chef 01</div>
          <div class="user-role">Shift Siang</div>
        </div>
        <svg class="chevron" viewBox="0 0 24 24"><polyline points="6 9 12 15 18 9"/></svg>
      </div>
      <div class="dropdown" id="profileDropdown">
        <div class="dropdown-header">
          <div class="dropdown-avatar">D1</div>
          <div>
            <div class="dropdown-name">Chef 01</div>
            <div class="dropdown-role">Shift Siang · Online</div>
          </div>
        </div>
        <div class="dropdown-body">
          <a href="/dapur/profil" class="dropdown-item">
            <div class="item-icon"><svg viewBox="0 0 24 24"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div>
            Profil Saya
          </a>
          <a href="/dapur/pengaturan" class="dropdown-item">
            <div class="item-icon"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg></div>
            Pengaturan
          </a>
          <a href="/dapur/ganti-sandi" class="dropdown-item">
...
<nav class="topnav">
  <div class="nav-container">
    <a href="/dapur/pesanan" class="nav-link {{ request()->is('dapur/pesanan') ? 'active' : '' }}">
  ...
  <div class="container">
    <div class="page-header">
      <div>
        ...
        <div class="page-sub" id="subTitle">Memuat pesanan...</div>
      </div>
      <span class="poll-badge">🟢 Live — update tiap 5 detik</span>
    </div>
...
function playBeep() {
  try {
    ...
    gain.gain.exponentialRampToValueAtTime(0.001, notifCtx.currentTime + 0.4);
    osc.start(); osc.stop(notifCtx.currentTime + 0.4);
  } catch(e) {}
}
...
function renderGrid(orders) {
  // ✅ Filter paid — kasir sudah konfirmasi bayar, siap dimasak
  const paid = orders.filter(o => o.status === 'paid');
  document.getElementById('badgePending').textContent = paid.length;
  document.getElementById('subTitle').textContent = paid.length + ' pesanan siap dimasak';

  const grid = document.getElementById('orderGrid');
  if (paid.length === 0) {
    grid.innerHTML = `<div class="empty-state">
      <div class="empty-icon">🎉</div>
      <div class="empty-title">Tidak ada pesanan masuk</div>
      <div class="empty-sub">Semua pesanan sudah diproses atau belum ada yang lunas.</div>
    </div>`;
    return;
  }
  ...
  grid.innerHTML = paid.map(o => {
    const items = o.items.map(i => `
      <div class="item-row">
        ...
        <div>
          <div class="item-name">${i.menu ? i.menu.nama : 'Menu #'+i.menu_id}</div>
          ${i.notes ? `<div class="item-notes">📝 ${i.notes}</div>` : ''}
        </div>
      </div>`).join('');

    const t = timeAgo(o.created_at);
    return `
    <div class="order-card" data-id="${o.queue_number}">
      <div class="order-card-top">
        <div class="oc-left">
          <div class="table-badge">🍽️</div>
          <div class="oc-info">
            <h3>🍽️ Meja ${o.table_number ?? '-'}</h3>
            <p>${o.queue_number} &nbsp;·&nbsp; <span class="pill pill-green">✅ Lunas</span></p>
          </div>
        </div>
        <div class="oc-time">${formatTime(o.created_at)}</div>
      </div>
      <div class="timer-wrap">
        <span class="timer-label">⏱ Menunggu sejak</span>
        <span class="timer ${t.warn ? 'warning' : ''}" data-time="${o.created_at}">${t.text}</span>
      </div>
      <div class="order-items">${items}</div>
      ${o.note ? `<div class="order-note">📝 <strong>Catatan:</strong> ${o.note}</div>` : ''}
      <div class="order-footer">
        <form action="/dapur/${o.${o.queue_number}id}/ambil" method="POST">
...
function updateTimers() {
  document.querySelectorAll('.timer[data-time]').forEach(el => {
    const t = timeAgo(el.dataset.time);
    el.textContent = t.text;
    el.classList.toggle('warning', t.warn);
  });
}
