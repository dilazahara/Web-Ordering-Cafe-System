...
/* ── CARD PAGE HEADER ── */
.card-page-header { padding: 24px 24px 20px; border-bottom: 1px solid #F0F0F0; background: linear-gradient(135deg, #eff4ff 0%, #fff 60%); }
...
.avatar-preview {
    ...
    font-size: 32px; font-weight: 800; color: white; background-color: #2563eb;
}
...
.btn-edit-avatar {
    ...
    background: #2563eb; color: white; border: 2.5px solid white; border-radius: 20px;
...
.btn-edit-avatar:hover { background: #1d4ed8; }
...
/* ── BADGE ── */
.info-badge { display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; background: #EFF4FF; color: #1d4ed8; }
...
.btn-save { padding: 10px 24px; border-radius: 10px; border: none; background: #2563eb; color: white; font-size: 14px; font-weight: 700; cursor: pointer; display: inline-flex; align-items: center; gap: 8px; transition: .2s; box-shadow: 0 4px 12px rgba(37,99,235,0.25); font-family: 'Plus Jakarta Sans', sans-serif; }
...
.pw-lock-icon svg { width: 26px; height: 26px; stroke: #2563eb; fill: none; stroke-width: 2; stroke-linecap: round; stroke-linejoin: round; }
...
.btn-verify:hover { background: #1d4ed8; transform: translateY(-1px); }
...
.btn-primary { padding: 10px 20px; border-radius: 10px; border: none; background: #2563eb; color: white; font-size: 14px; font-weight: 700; cursor: pointer; transition: background .15s; font-family: 'Plus Jakarta Sans', sans-serif; }
...
@section('content')
    ...
    <form action="{{ route('kasir.account.update') }}" method="POST" id="formProfil" novalidate>
        @csrf
        @method('PUT')
        ...
        <div class="card">
            ...
            <div class="card-section">
                ...
                <div class="avatar-row">
                    ...
                    @php
                        ...
                        $initial   = strtoupper(substr(auth()->user()->name ?? 'K', 0, 1));
                    @endphp
                    ...
                    <div class="avatar-info">
                        <h3 id="displayName">{{ auth()->user()->name ?? 'Kasir' }}</h3>
                        <p>{{ auth()->user()->email ?? '' }}</p>
                ...
                <div class="form-grid-2">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input"
                               value="{{ ucfirst(auth()->user()->role ?? 'Kasir') }}" readonly>
                    </div>
                    <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-password-wrap">
                                    ...
                                    <button type="button" class="btn-eye" onclick="togglePwd('newPassword', this)">
                                        <svg viewBox="0 0 24 24" id="eyeNew"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                                    </button>
                                </div>
            ...
            <div class="card-footer">
                <a href="{{ route('kasir.dashboard') }}" class="btn-cancel">Kembali</a>
    ...
    <div class="modal-box">
        ...
        <div class="verify-body">
            <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">
                <div style="width:44px;height:44px;border-radius:50%;background:#eff4ff;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                    <svg viewBox="0 0 24 24" fill="none" stroke="#2563eb" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:22px;height:22px;">
...
const AVATAR_AJAX_URL = "{{ route('kasir.account.update-avatar') }}";
const VERIFY_URL      = "{{ route('kasir.account.verify-password') }}";
...
/* ══════════════════════════════════
   AJAX — kirim ke server & update header
══════════════════════════════════ */
...
/* Update avatar di header layout */
function updateHeaderAvatar(url) {
    document.querySelectorAll('.avatar, .dropdown-avatar').forEach(function(el) {
