...
.form-input, .form-select, .form-textarea { width: 100%; padding: 12px 16px; border-radius: 12px; border: 1.5px solid #e5e7eb; background: #f9fafb; font-size: 14px; color: #111827; outline: none; transition: .2s; font-family: 'Plus Jakarta Sans', sans-serif; }
...
.img-current {
    ...
    background: #f3f4f6; height: 180px; margin-bottom: 10px;
}
...
.img-upload-new { border: 2px dashed #e5e7eb; border-radius: 14px; padding: 20px; text-align: center; cursor: pointer; background: #fafafa; display: none; position: relative; transition: all .2s; }
...
.img-preview-new { width: 100%; height: 180px; object-fit: cover; border-radius: 10px; display: none; margin-top: 10px; }
.img-preview-new.show { display: block; }
.change-img-btn { width: 100%; padding: 10px; border-radius: 11px; border: 1.5px dashed #d1d5db; background: transparent; color: #6b7280; font-size: 12.5px; font-weight: 700; cursor: pointer; font-family: 'Plus Jakarta Sans', sans-serif; transition: all .15s; display: flex; align-items: center; justify-content: center; gap: 6px; }
...
/* ADDON */
.addon-item { display: flex; align-items: flex-start; gap: 12px; padding: 13px 15px; border-radius: 13px; border: 1.5px solid #f1f5f9; background: #fafafa; cursor: pointer; transition: all .2s; margin-bottom: 8px; }
.addon-item:hover { border-color: #ddd6fe; background: #f5f3ff; }
.addon-item.selected { border-color: #8b5cf6; background: #f5f3ff; }
.addon-item input[type=checkbox] { appearance: none; -webkit-appearance: none; width: 20px; height: 20px; flex-shrink: 0; margin-top: 1px; border: 2px solid #d1d5db; border-radius: 6px; background: white; cursor: pointer; transition: all .18s; }
.addon-item input[type=checkbox]:checked { background: #8b5cf6; border-color: #8b5cf6; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 20 20' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M5 10l3.5 3.5L15 7' stroke='white' stroke-width='2.2' stroke-linecap='round' stroke-linejoin='round' fill='none'/%3E%3C/svg%3E"); background-repeat: no-repeat; background-position: center; }
    ...
    <div class="card">
        ...
        <div class="form-grid-2" style="margin-bottom:14px;">
            ...
            <div class="form-group" style="margin-bottom:0;">
                ...
                <input type="number" name="price" class="form-input" value="{{ $menu->price }}" required>
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <textarea name="description" class="form-textarea" rows="3">{{ $menu->description }}</textarea>
        </div>
        ...
        <div class="form-group">
            <div class="img-current" id="currentImgWrap">
                <img id="currentImg" src="{{ $menu->image ? asset('storage/'.$menu->image) : 'https://via.placeholder.com/560x180?text=No+Image' }}" alt="{{ $menu->name }}">
                <div class="img-overlay" onclick="showUpload()">
                    <i data-lucide="camera" style="width:24px;height:24px;stroke:white;"></i>
                    <span>Ganti Foto</span>
                </div>
            </div>
            <button type="button" class="change-img-btn" id="changeBtnEl" onclick="showUpload()">
                <i data-lucide="upload" style="width:14px;height:14px;"></i> Ganti foto menu
            </button>
            <div class="img-upload-new" id="uploadNew">
                <input type="file" name="image" accept=".jpg,.jpeg,.png" onchange="previewNewImage(event)">
                <div style="color:#d1d5db;margin-bottom:8px;"><i data-lucide="image" style="width:28px;height:28px;"></i></div>
...
function previewNewImage(event) {
    ...
    reader.onload = function() {
        ...
        document.getElementById('uploadNew').style.padding = '0';
        document.getElementById('uploadNew').style.border = 'none';
    };
...
function confirmDelete() {
    if (confirm('Yakin ingin menghapus menu ini? Tindakan ini tidak bisa dibatalkan.')) {
