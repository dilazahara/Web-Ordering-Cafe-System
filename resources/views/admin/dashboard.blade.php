...
<head>
    ...
    {{-- Google Fonts: Inter (konsisten dgn semua halaman) --}}
...
<div class="main">
    ...
    <div class="box" style="margin-top:20px;">
        <div class="box-header">
            ...
            <a href="/admin/order" style="font-size:var(--text-sm);color:#7c3aed;font-weight:700;text-decoration:none;">
                Lihat semua →
            </a>
        </div>
        ...
        <div class="table-responsive">
            <table class="rtable" id="recentOrdersTable">
                <thead>
                    <tr>
                        <th style="padding-left:var(--space-lg);">ID Order</th>
                        <th>Meja</th>
                        <th>Item</th>
                        <th>Total</th>
                        <th>Metode Bayar</th>
                ...
                <tr>
                    <td style="padding-left:var(--space-lg);">
                        <span style="font-weight:800;color:#7c3aed;font-size:var(--text-base);">
                            {{ $order->queue_number ?: 'A-' . str_pad($order->id, 3, '0', STR_PAD_LEFT) }}
                        </span>
                    </td>
                    <td>
                        <span style="background:var(--border-light);padding:4px 10px;border-radius:var(--radius-sm);font-size:var(--text-sm);font-weight:700;color:var(--text-light);">
                    ...
                    <td>
                        @if($order->items && $order->items->count() > 0)
                            ...
                            @if($order->items->count() > 2)
                                <span style="color:var(--text-muted);font-size:var(--text-sm);">+{{ $order->items->count()-2 }} lagi</span>
                            @endif
                        @else
                            <span style="font-size:var(--text-base);color:var(--text-muted);">-</span>
                        @endif
                    </td>
                    <td style="font-weight:600;font-size:var(--text-base);color:var(--text-dark);">
                        Rp {{ number_format($order->total ?? 0, 0, ',', '.') }}
                    </td>
                    <td>
                        @if($order->payment_method === 'cash')
                            <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:var(--radius-sm);font-size:var(--text-xs);font-weight:700;background:#fef3c7;color:#92400e;border:1px solid #fde68a;">💵 Cash</span>
                        @elseif($order->payment_method === 'qris')
                            <span style="display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:var(--radius-sm);font-size:var(--text-xs);font-weight:700;background:#ede9fe;color:#5b21b6;border:1px solid #ddd6fe;">📱 QRIS</span>
                        @else
                            <span style="font-size:var(--text-sm);color:var(--text-light);text-transform:capitalize;">{{ $order->payment_method ?? 'Tunai' }}</span>
                        @endif
                    </td>
                    <td>
                        @php
                    ...
                    <td style="font-size:var(--text-sm);color:var(--text-muted);white-space:nowrap;">
